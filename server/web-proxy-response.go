...
func getWebProxyResponse(resp http.ResponseWriter, isTextFormat bool) *webProxyResponse {
	g := &webProxyResponse{
...
func (w *webProxyResponse) Header() http.Header {
	return w.headers
}

func (w *webProxyResponse) Write(b []byte) (int, error) {
	if !w.wroteHeaders {
		w.prepareHeaders()
	}
...
func (w *webProxyResponse) WriteHeader(code int) {
	w.prepareHeaders()
	w.wrapped.WriteHeader(code)
	w.wroteHeaders = true
}

func (w *webProxyResponse) Flush() {
...
func (w *webProxyResponse) prepareHeaders() {
	wh := w.wrapped.Header()
	copyHeader(
		wh, w.headers,
		skipKeys("trailer"),
...
func (w *webProxyResponse) finishRequest(req *http.Request) {
	if w.wroteHeaders || w.wroteBody {
...
func (w *webProxyResponse) copyTrailersToPayload() {
